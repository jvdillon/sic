...
def estimate_lipschitz(exp, n_puzzles=100):
    ...
    with torch.no_grad():
        for batch in exp.make_test_loader():
            ...
            for i in range(min(valid_count, B)):
                ...
                # Get f(z_H)
                with torch.autocast(device_type=exp.device.type, dtype=exp.dtype):
                    out0 = exp._eval_forward(inp_i, zh_i, zl_i, pid_i)
                ...
                for _ in range(N_PERTURBATIONS):
                    ...
                    with torch.autocast(device_type=exp.device.type, dtype=exp.dtype):
                        out_p = exp._eval_forward(inp_i, zh_pert, zl_i, pid_i)
...
def stats(vals):
    ...
    return (s[0], s[n//4], s[n//2], s[3*n//4], s[-1],
            sum(s)/n, sum(1 for v in s if v > 1.0))
...
print(f"{'':>6s}  {'min':>6s}  {'p25':>6s}  {'p50':>6s}  {'p75':>6s}  {'max':>6s}  {'mean':>6s}  {'>1':>4s}")
print(f"  PRE   {pre_s[0]:6.3f}  {pre_s[1]:6.3f}  {pre_s[2]:6.3f}  {pre_s[3]:6.3f}  {pre_s[4]:6.3f}  {pre_s[5]:6.3f}  {pre_s[6]:4d}")
print(f"  POST  {post_s[0]:6.3f}  {post_s[1]:6.3f}  {post_s[2]:6.3f}  {post_s[3]:6.3f}  {post_s[4]:6.3f}  {post_s[5]:6.3f}  {post_s[6]:4d}")
