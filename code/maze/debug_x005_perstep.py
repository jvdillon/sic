...
def per_step_eval(exp: Any) -> Any:
    ...
    with torch.no_grad():
        for batch in exp.make_test_loader():
            ...
            all_step_correct.extend(
                (grid_len, step_corrects[i].cpu()) for i in range(valid_count)
            )
    exp.model.train()
...
print(
    f"{'step':>4s}  {'PRE_cell':>9s} {'PRE_puz':>9s}  {'POST_cell':>9s} {'POST_puz':>9s}  {'delta_puz':>9s}"
)
n = len(pre_data)
for s in range(MAX_STEPS):
    ...
    print(
        f"  {s + 1:2d}    {pre_ca:7.2f}%  {pre_pa:7.2f}%   {post_ca:7.2f}%  {post_pa:7.2f}%   {post_pa - pre_pa:+7.2f}%"
    )

# Per-puzzle best-step analysis
print("\n=== BEST-STEP ANALYSIS ===")
pre_best = sum(
    1 for d in pre_data if any(d[1][s].item() == d[0] for s in range(MAX_STEPS))
)
post_best = sum(
    1 for d in post_data if any(d[1][s].item() == d[0] for s in range(MAX_STEPS))
)
print(f"  PRE  puzzles correct at ANY step: {pre_best}/{n} ({100 * pre_best / n:.1f}%)")
print(
    f"  POST puzzles correct at ANY step: {post_best}/{n} ({100 * post_best / n:.1f}%)"
)

# Which step is best for each model?
pre_step_puz = [
    sum(1 for d in pre_data if d[1][s].item() == d[0]) for s in range(MAX_STEPS)
]
post_step_puz = [
    sum(1 for d in post_data if d[1][s].item() == d[0]) for s in range(MAX_STEPS)
]
...
print(
    f"  POST best step: {post_best_step + 1} ({post_step_puz[post_best_step]} puzzles)"
)
