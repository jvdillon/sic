    ...
    with torch.no_grad():
        for batch in exp.make_test_loader():
            ...
            for i in range(valid_count):
                results.append((deltas[i], corrects[i], grid_len))
    exp.model.train()
    return results
...
print(f"lost={len(lost)} stayed_right={len(stayed_right)} "
      f"stayed_wrong={len(stayed_wrong)} gained={len(gained)}")


def group_mean_delta(indices, data, step_idx):
    if not indices:
        return 0.0
...
print(f"{'step':>6s}  {'lost':>8s}  {'stayed_R':>8s}  {'stayed_W':>8s}  {'ratio_L/R':>10s}")
for s in range(MAX_STEPS - 1):
    l = group_mean_delta(lost, post, s)
    r = group_mean_delta(stayed_right, post, s)
    w = group_mean_delta(stayed_wrong, post, s)
    ratio = l / r if r > 0 else float("inf")
    print(f"  {s+1}->{s+2}  {l:8.4f}  {r:8.4f}  {w:8.4f}  {ratio:10.3f}")

# Same for PRE
print("\n=== MEAN z_H DELTA AT LATE STEPS (PRE model) ===")
print(f"{'step':>6s}  {'lost':>8s}  {'stayed_R':>8s}  {'stayed_W':>8s}  {'ratio_L/R':>10s}")
for s in range(MAX_STEPS - 1):
    l = group_mean_delta(lost, pre, s)
...
for label, indices in [("lost", lost), ("stayed_right", stayed_right),
    ...
    if vals:
        ...
        print(f"  {label:12s}: p25={p25:.4f} p50={p50:.4f} p75={p75:.4f} "
