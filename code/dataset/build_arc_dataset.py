...
def arc_grid_to_np(grid: list[list[int]]) -> np.ndarray:
    ...
    # Shape check
    assert arr.ndim == 2
    assert arr.shape[0] <= ARCMaxGridSize
    assert arr.shape[1] <= ARCMaxGridSize
    # Element check
...
def np_grid_to_seq_translational_augment(
    ...
    if do_translation:
        pad_r = rng.integers(0, ARCMaxGridSize - max(inp.shape[0], out.shape[0]) + 1)
        pad_c = rng.integers(0, ARCMaxGridSize - max(inp.shape[1], out.shape[1]) + 1)
    else:
        pad_r = pad_c = 0
...
def puzzle_hash(puzzle: dict[tuple[str, str], ARCPuzzle]) -> str:
    ...
    hashes = []
    for example in puzzle.values():
        for inp, label in example.examples:
            hashes.append(f"{grid_hash(inp)}|{grid_hash(label)}")

    hashes.sort()
    return hashlib.sha256("|".join(hashes).encode()).hexdigest()


def aug(name: str) -> tuple[str, Callable[[np.ndarray], np.ndarray]]:
    # Augment plan
    ...
    name_with_aug_repr = (
        f"{name}{PuzzleIdSeparator}t{trans_id}{PuzzleIdSeparator}"
        f"{''.join(str(x) for x in mapping)}"
    )
...
def inverse_aug(name: str) -> tuple[str, Callable[[np.ndarray], np.ndarray]]:
    ...
    trans_id_int = int(trans_id[1:])  # Remove "t" letter
...
def convert_single_arc_puzzle(
    ...
    name: str,
    puzzle: dict[str, list[dict[str, Any]]],
    aug_count: int,
        ...
        for _trial in range(ARCAugmentRetriesFactor * aug_count):
            ...
            augmented = {
                dest: ARCPuzzle(
                    aug_name,
                    [
                        (_map_grid(inp), _map_grid(label))
                        for (inp, label) in puz.examples
                    ],
                )
    ...
    for dest in dests:
        ...
        results[dest_split][dest_set].append([conv[dest] for conv in group])
...
def load_puzzles_arcagi(
    ...
    test_examples_map: dict[str, list[tuple[float, tuple[str, str]]]] = {
    ...
    for subset_name in config.subsets:
        ...
        sols_path = Path(f"{config.input_file_prefix}_{subset_name}_solutions.json")
        if sols_path.is_file():
            with sols_path.open() as f:
                sols = json.load(f)
...
def convert_dataset(config: DataProcessConfig) -> None:
    ...
    for split_name, split in data.items():
        output_path = Path(config.output_dir) / split_name
        ...
        for subset_name, subset in split.items():  # "all" is the only subset
            ...
            for group in subset:
                for puzzle in group:
                    # Push puzzle
                    no_aug_id = rng.integers(0, len(puzzle.examples))
                    for _idx_ex, (inp_grid, out_grid) in enumerate(puzzle.examples):
            ...
            for k, v in results.items():
                ...
                np.save(output_path / f"{subset_name}__{k}.npy", arr)
        ...
        with (output_path / "dataset.json").open("w") as f:
    ...
    # Save Test Puzzles
    with (output_dir / "test_puzzles.json").open("w") as f:
