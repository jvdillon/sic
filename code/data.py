...
class PuzzleDatasetLegacy:
    """GPU-cached puzzle dataset with stratified sampling.
    ...
    def __init__(
        ...
        if self.stratified:
            self.group_starts = group_indices[:-1].to(device)
            self.n_groups = len(self.group_starts)
            group_sizes = group_indices[1:] - group_indices[:-1]
            # Check if all groups have uniform size (b4b compat with legacy randint)
            if (group_sizes == group_sizes[0]).all():
                self.augs_per_group: int | None = int(group_sizes[0])
                self.group_sizes: Tensor | None = None
            else:
                self.augs_per_group = None
                self.group_sizes = group_sizes.to(device)

    def __iter__(self):
        if self.stratified:
            if self.augs_per_group is not None:
                # Uniform groups: use randint for b4b compat with legacy code
                ...
                # Variable groups: generate uniform [0,1) then scale by group size
                assert self.group_sizes is not None
                aug_offsets = (
                    torch.rand(self.n_groups, device=self.device) * self.group_sizes
                ).long()
        ...
        for i in range(0, n_samples, self.batch_size):
            ...
            if valid_count < self.batch_size:
                pad = self.batch_size - valid_count
                inputs = torch.cat([inputs, inputs.new_zeros(pad, inputs.shape[1])])
                labels = torch.cat([labels, labels.new_zeros(pad, labels.shape[1])])
