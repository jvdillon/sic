...
winner's $z_H$ is copied to all chains (\Cref{alg:wta}, line 10). For $z_L$, we
...
\paragraph{Schur complement structure (intuition).} The implicit gradient through
this bilevel system reveals an elegant structure. Writing the joint state
...
the inner equilibrium $(I-J_L)^{-1}$. This creates nested Neumann series:
the inner equilibrium ``solves'' for $z_L^*$ given $z_H$, then the outer loop
optimizes the simpler problem of finding good $z_H$---divide and conquer.
In practice, TRM truncates these series, but the bilevel structure may make
truncation less harmful: deep effective computation with shallow gradient paths.
