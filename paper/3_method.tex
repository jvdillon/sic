...
Why does lowest loss correlate with fastest halting? The mechanism is
convergence dynamics. When iterative refinement encounters no contradictions,
$z_L$ stabilizes quickly---the fixed-point iteration has a large basin of
attraction. This rapid stabilization produces both correct outputs (low loss)
and confident halting signals (the model detects its own convergence). Conversely,
when refinement oscillates or drifts, both accuracy and halting confidence
suffer. Loss and halting time are thus downstream effects of the same cause:
