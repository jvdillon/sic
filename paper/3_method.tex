...
standard SwiGLU activation. Standard SwiGLU computes $\text{SiLU}(g) \odot v$
where $[g, v] = xW$ (split in half). Our modification applies normalization
before the down-projection: $\text{SwiGLU}_\text{mod}(x) = \sigma(g) \odot
\text{RMSNorm}(g \odot v)$, using sigmoid rather than SiLU for the gate. This
bounds the pre-activation magnitudes, preventing Muon's orthogonalized updates
from causing training to diverge.
...
\paragraph{Why shared $z_H$.} The \texttt{copy\_top1} policy copies the
