...
We apply this principle to ensembles of Tiny Recursive Models (TRM)~\citep{jolicoeur2025trm}. By basing the ensemble prediction solely on the first to halt rather than averaging predictions, we achieve ${\sim}$97\% puzzle accuracy on Sudoku-Extreme while using 10$\times$ less compute than probability averaging (the baseline achieves ${\sim}$86\%). Inference speed is an implicit indication of confidence.

% Source: x182{a,b,c,d}.log (trunc\_randn): 96.85% ± 0.58%
