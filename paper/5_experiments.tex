...
Training completes in 1.5 hours ($K{=}1$, bs${=}$384, ${\sim}$16GB) to 6 hours
($K{=}4$, bs${=}$192, ${\sim}$30GB) for 36k steps.
...
%
% KEY FINDING: randn (K=∞ effective) ≈ Buffer (K=4 learned) → 96.9% both!
...
% - x182{a,b,c,d} (randn): 96.85% ± 0.58%, range 96.2-97.6%
...
K=4 randn init & 98.6\% & 96.9$\pm$0.6\% & 1$\times$ \\  % x182a.log, x182b.log, x182c.log, x182d.log
...
\begin{enumerate}
  ...
  \item \textbf{K=4 $\approx$ K=$\infty$.} Random init (fresh randn each forward
    ...
    % Source: x182{a,b,c,d} (randn) vs x182{j,k,l,m} (Buffer)
...
% Mean: 96.85% ± 0.58% (randn); x182{j,k,l,m} Buffer achieves same 96.9%
\begin{figure}[t]
\centering
...
\begin{itemize}
  \item Baseline ($K{=}1$, 36k steps, bs${=}$384): 1.5 hours, ${\sim}$16GB VRAM
  \item WTA ($K{=}4$, 36k steps, bs${=}$192): 6 hours, ${\sim}$30GB VRAM
...
% Source: x182a.log z_L cosine similarity analysis
\begin{figure}[t]
\centering
\begin{tikzpicture}[scale=0.8]
  \begin{axis}[
