...
%
% KEY FINDING: fresh init ≈ fixed init → 96.9% both!
...
Fixed init & 98.6\% & 96.9$\pm$0.8\% & 1$\times$ \\  % x182j.log, x182k.log, x182l.log, x182m.log
\bottomrule
\end{tabular}
...
parallel $z_L$ heads and achieve \textbf{96.9\%} single-pass accuracy---matching
TTA-8 (97.3\%) without test-time augmentation. Fresh init (resample each step)
and fixed init (cached in buffer) perform identically.}
\label{tab:main}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{Initialization policy.}
      Fresh init (resample $z_L$ each forward pass) achieves 96.85\% $\pm$ 0.58\%;
        ...
        Fresh is marginally better with tighter variance.
...
% Mean: 96.85% ± 0.58% (fresh init); x182{j,k,l,m} fixed init achieves same 96.9%
\begin{figure}[t]
\centering
...
96.2--97.6\% by 36k steps. Fixed init shows identical final accuracy
