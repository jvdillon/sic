...
\begin{enumerate}
  \item \textbf{Selection is the bottleneck.}
      A TTA diagnostic reveals 89\% of baseline failures are selection problems
        ...
        permutation). The ceiling is 99.4\%, not 86\%.
        % Source: analysis/digit_perm_inference.py - 1820/2037 failures solved by TTA
  \item \textbf{WTA training internalizes diversity.}
      K=4 z\_L heads achieve 96.9\% single-pass accuracy---nearly matching
        TTA-8 (97.3\%) without any test-time augmentation.
        % Source: x182{a,b,c,d,j,k,l,m}.log
  \item \textbf{Initialization.}
      Repeated init (trunc\_randn for every new $z_L$) and fixed init
        (trunc\_randn copied from buffer) achieves 96.85\% $\pm$ 0.58\%
        and 96.80 $\pm$ 0.81\%, resp.
