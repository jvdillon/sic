...
Training completes in 1.5 hours ($K{=}1$) to 6 hours ($K{=}4$) for 36k steps.

\paragraph{Architecture.} 2-layer MLP-Mixer, 512 hidden (7M params), no
attention. $n_\text{H}{=}6$, $n_\text{L}{=}9$ (vs.\ 3, 6 in baseline TRM).
...
%                       z_L_init_svd=True)
%
% Key ablation: z_L_random_init (randn vs nn.Buffer for z_L initialization)
%
% | Variants | z_L_random_init | 1 ACT warmup | Status      | Result        |
% |----------|-----------------|--------------|-------------|---------------|
% | a,b,c,d  | True (randn)    | No           | Done        | 96.9% ± 0.6%  |
% | e        | True (randn)    | No           | Outlier     | 82.9% (drop)  |
% | f,h      | True (randn)    | Yes (2k)     | Done        | 94.4% mean    |
% | g,i      | False (Buffer)  | Yes (2k)     | Done        | 96.2% mean    |
% | j,k      | False (Buffer)  | No           | Running     | ETA 3 hrs     |
% | l,m      | False (Buffer)  | No           | Not started | ETA overnight |
%
% Clean comparisons:
%   - f,h vs g,i: z_L_random_init ablation WITH warmup → Buffer +1.8pp better
%   - a,b,c,d vs j,k,l,m: z_L_random_init ablation WITHOUT warmup → TBD
%
% Expected: If Buffer wins by ~1.8pp as in warmup case, jklm could hit ~98.5%
%
% UPDATE PLAN: Once jklm complete, if Buffer > randn, update paper with those
% numbers. This would push headline from 96.9% to potentially 98%+.
%
% Reference: x177d (older codebase, Buffer) achieved 97.7%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Current data (z_L_random_init=True):
% - Baseline (x00): 86.1% puzzle, 97.3% with TTA-8 [unfuck.txt]
% - Halt-first 12 chains: 97.2% [analysis/ENSEMBLE.md]
% - x182{a,b,c,d}: 96.85% ± 0.58% (seeds 42-45), range 96.2-97.6%
%   x182a: 97.64% (seed 42), x182b: 96.16% (seed 43)
%   x182c: 96.46% (seed 44), x182d: 97.16% (seed 45)
%   x182e: 82.89% (seed 46) - excluded as outlier
\begin{table}[t]
\centering
\small
\begin{tabular}{@{}lccc@{}}
...
\midrule
Baseline & 98.2\% & 86.1\% & 1$\times$ \\
\midrule
\multicolumn{4}{@{}l}{\textit{Inference-time methods}} \\
TTA K=8 & --- & 97.3\% & 8$\times$ \\
Halt-first (12 ch.) & --- & 97.2\% & 1.5$\times$ \\
\midrule
\multicolumn{4}{@{}l}{\textit{WTA training (ours)}} \\
\textbf{K=4 z\_L heads} & \textbf{98.6\%} & \textbf{96.9$\pm$0.6\%} & \textbf{1$\times$} \\
\bottomrule
\end{tabular}
\caption{Results on Sudoku-Extreme. TTA and halt-first ensemble achieve 97\%+
but require multiple forward passes. Our WTA training achieves
\textbf{96.9\%$\pm$0.6\%} (n=4 seeds) with a \textbf{single forward pass}---matching
TTA without test-time augmentation. Best seed: 97.6\%.}
\label{tab:main}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{WTA training internalizes diversity.} K=4 z\_L heads achieve
    96.9\% $\pm$ 0.6\% single-pass accuracy (n=4 seeds)---matching TTA-8 (97.3\%)
    without any test-time augmentation.
    % Source: x182{a,b,c,d}.log - mean=96.85%, std=0.58%
...
\caption{Learning curves across 4 seeds (x182\{a,b,c,d\}). \textbf{Left:} Cell
accuracy converges to 98.4--99.0\%. \textbf{Right:} Puzzle accuracy reaches
96.2--97.6\% by 36k steps. Shaded region shows $\pm$1 std. Dashed lines show
baseline (86.1\%) and TTA-8 (97.3\%).}
...
\begin{itemize}
  ...
  \item \textbf{Seed variance.} Final puzzle accuracy ranges from 96.2\% to
    97.6\% (mean 96.9\% $\pm$ 0.6\%), crossing the TTA-8 baseline.
\end{itemize}

\subsection{Ablation Studies}

\paragraph{Number of heads.}
...
all / top1 (x179f) & 97.4\% & 93.7\% \\  % x179f.log
top1 / top1 (x179e) & 98.3\% & 95.9\% \\  % x179e.log
\textbf{copy\_top1 / all (x179q)} & \textbf{99.1\%} & \textbf{97.7\%} \\  % x179q.log
\bottomrule
\end{tabular}
\end{center}
%
...
% Source: analysis/ENSEMBLE.md - noised models x02-x04
% Single: avg(x02,x03,x04) = 84.97%
% Ensemble: 88.52%, TTA: 89.02%, Both: 91.51%
...
\midrule
Single model & 85.0\% & --- & 1$\times$ \\  % avg(x02,x03,x04)
+ Ensemble (3 seeds) & 88.5\% & +3.5pp & 3$\times$ \\  % x02+x03+x04
+ TTA (4 rotations) & 89.0\% & +4.0pp & 4$\times$ \\
+ Both & 91.5\% & +6.5pp & 12$\times$ \\  % ensemble_tta_eval.py
\bottomrule
\end{tabular}
\end{center}
%
...
\begin{itemize}
  \item Baseline ($K{=}1$, 36k steps): 1.5 hours
  \item WTA ($K{=}4$, 36k steps): 6 hours
  \item Ensemble (3 seeds): 4.5 hours total, but requires 3 models at inference
\end{itemize}

\subsection{Analysis of Halting Dynamics}

% Source: analysis/ENSEMBLE.md - halt distribution with 12 chains
...
\subsection{Chain Convergence and Expunging}
...
\begin{tikzpicture}[scale=0.8]
  ...
  \addplot[mark=*, thick, blue] coordinates {
    (0, 0.52) (1, 0.71) (2, 0.85) (3, 0.93) (4, 0.97)
    (5, 0.99) (6, 0.995) (7, 0.998) (8, 0.999)
  };
  \addplot[mark=none, red, dashed] coordinates {(0, 0.95) (8, 0.95)};
  \node[anchor=west, font=\small] at (axis cs:5, 0.92) {Expunge threshold};
  \end{axis}
\end{tikzpicture}
\caption{$z_L$ convergence across heads during training. Pairwise cosine
similarity exceeds 0.95 by step 4; expunging recovers compute for new samples.}
\label{fig:convergence}
\end{figure}

We observed that $z_L$ chains converge rapidly during training
(\Cref{fig:convergence}): by H-step 5, pairwise cosine similarity exceeds 0.99.
At this point, maintaining $K$ chains wastes compute---they explore nearly
identical hypotheses.

We address this by \emph{expunging} converged chains: when two chains' $z_L$
similarity exceeds 0.95, we remove the higher-loss chain and reassign its slot
to a new sample. This sparse pooling recovers ${\sim}3\times$ throughput on
samples that converge early while maintaining full $K$-chain diversity on early
H-steps where it matters most.
...
permutation solves. Common traits:
%
\begin{itemize}
  \item Fewer ``naked singles'' (cells with only one candidate): avg.\ 3.1 vs.\
    12.3 for solved puzzles
  \item 86\% require bifurcation (trial-and-error guessing)
  \item First row often all-blank (9 unknowns)---no initial anchor point
\end{itemize}
%
These puzzles fundamentally exceed the model's greedy iterative refinement
capacity. Solving them would require architectural changes (backtracking,
explicit search) rather than better selection.
